...
WebUI.callTestCase(findTestCase('AHI/_include/get_screenPath'), [('productLine') : 'AHI', ('carrierName') : 'Dental - Delta Dental'], 
...
WebUI.click(findTestObject('mobileSanity/Dental/filter/btn_delta'))
...
for (second = 0; second < 30; second++) {
    try {
        if (selenium.isElementPresent('id=js-app-confirmation')) {
            break
        }
    }
    catch (Exception e) {
