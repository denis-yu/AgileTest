...
WebUI.callTestCase(findTestCase('AHI/_include/get_screenPath'), [('productLine') : 'HP', ('carrierName') : 'Small Business'], FailureHandling.CONTINUE_ON_FAILURE)

int second

WebUI.openBrowser(GlobalVariable.ENV+'/small-business-health-insurance')
...
for (second = 0; second < 60; second++) {
    try {
        if (selenium.isElementPresent('//div/h2[contains(text(),\'Ways to Enroll or Learn More\')]')) {
            break
        }
    }
    catch (Exception e) {
