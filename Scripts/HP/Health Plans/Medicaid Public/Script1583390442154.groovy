...
WebUI.callTestCase(findTestCase('AHI/_include/get_screenPath'), [('productLine') : 'HP', ('carrierName') : 'Medicaid Public'], FailureHandling.CONTINUE_ON_FAILURE)

int second

WebUI.openBrowser(GlobalVariable.ENV+'/medicaid-public-plans')
...
for (second = 0; second < 60; second++) {
    try {
        if (selenium.isElementPresent('//div/h2[contains(text(),\'Ways to Learn More\')]')) {
            break
        }
    }
    catch (Exception e) {
